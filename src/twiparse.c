...
int parse_statuses_json(json_t *statuses_root, statuses *tl){
    if(!statuses_root || !tl)
        return -1;

    if(!statuses_root || !json_is_array(statuses_root))
        return -1;

    int nr_statuses = 0;
    struct status_node *prev = NULL;
    for(int i = 0; i<json_array_size(statuses_root);i++){
        json_t *status_root = json_array_get(statuses_root, i);
        ...
        sn->prev = prev;
        parse_status_json(status_root, &(sn->st));
        prev = sn;
    }
...
int parse_timeline(char *data, statuses *tl){
    ...
    return parse_statuses_json(root, tl);
}
